<template>
  ...
  <div :class="{right: true, show: newCon, edit: view, row: true}">
    ...
    <form>
      ...
      <div class = "form-group row" v-for = "(address, index) in contact.address" :key="index">
...
export default {
  ...
  methods: {
    ...
    validateEmail: function () {
      var good = true
      this.contact.email.forEach((emailAddress) => {
        if (!(/[\w.+-]+@[\w.+-]+\.[a-zA-Z0-9]{2,4}(,\s*)*/ig.test(emailAddress))) {
          good = false
        }
      })
    ...
    validatePhoneNumber: function () {
      var good = true
      this.contact.phoneNumber.forEach((number) => {
